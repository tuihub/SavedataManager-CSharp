    ...
    public partial class MainForm : Form
    {
        ...
        private void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.OK && String.IsNullOrEmpty(filePath) == false)
            {
                ...
                {
                    foreach (var pattern in entry.FilePatterns)
                    {
                        entryDataGridView.AddItem(entry.Id, pattern.Type, pattern.Pattern, entry.ClearBaseDirBeforeRestore, entry.BaseDirMode, entry.BaseDir);
                    }
                }
            }
        }
